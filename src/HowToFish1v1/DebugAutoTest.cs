...
namespace HowToFish1v1
{
    ...
    /// start, kill the local player during the first live round to force a reset, then quit after round 2 goes live.
    ...
    internal static class DebugAutoTest
    {
        ...
        public static void Update()
        {
            ...
            switch (_step)
            {
                case Step.WaitPlayer:
                    ...
                    Plugin.Host.SetLocalLoadout(ids, true);
                    Plugin.Host.Start();
                    _step = Step.Started;
                    _at = Time.time + 30f;
                    break;
                ...
                case Step.KilledSelf:
                    if (_liveCount >= 2) { _step = Step.WaitLive2; _at = Time.time + 4f; }
                    else if (Time.time > _at) { Plugin.Log.LogError("AutoTest: round 2 never went live"); _step = Step.Done; }
                    break;
                case Step.WaitLive2:
                    if (Time.time < _at) return;
                    LogPlayer("round 2 live");
                    Plugin.Log.LogInfo("AutoTest: quitting 1v1");
                    Plugin.Host.Quit();
                    _step = Step.Quit;
                    _at = Time.time + 14f;
                    break;
        ...
        private static void LogPlayer(string when)
        {
            ...
            int ammo = p.Holding.HeldItem && p.Holding.HeldItem.Weapon ? p.Holding.HeldItem.Weapon.Ammo : -1;
